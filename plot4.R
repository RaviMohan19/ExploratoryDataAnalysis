...
## plot 4 is a multivariate 
par(mfrow=c(2,2))
with(power_subset,{
  plot(power_subset$Time,as.numeric(as.character(power_subset$Global_active_power)),type="l",  xlab="Day",ylab="Global Active Power")
  plot(power_subset$Time,as.numeric(as.character(power_subset$Voltage)), type="l",xlab="datetime",ylab="Voltage")
  plot(power_subset$Time,power_subset$Sub_metering_1,type="n",xlab="Day",ylab="Energy sub metering")
